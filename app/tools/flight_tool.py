"""
Replace this with your existing flight search implementation.
Kept here only so the project structure is complete and importable.

IMPORTANT for production: make sure whatever HTTP client this uses has an
explicit timeout set (e.g. requests.get(url, timeout=10)) - an unbounded
network call inside a @tool is the most common cause of a hung worker.
"""
